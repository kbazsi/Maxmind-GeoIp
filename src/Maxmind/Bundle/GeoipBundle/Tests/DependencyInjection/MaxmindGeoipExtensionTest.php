...
class MaxmindGeoipExtensionTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testMinimalConfig()
    {
        ...
        $datFilePath = $this
            ->configuration
            ->getParameter('maxmind_geoip_data_file_path')
        ;
        $configuredFolder = realpath(
            substr($datFilePath, 0, strrpos($datFilePath, "/"))
        );
        $expectedFolder = realpath(__DIR__."/../../../../../../data/");

        $this->assertEquals(
            $expectedFolder,
            $configuredFolder, "Unexpected file path."
        );
    }
    ...
    public function testFullConfig()
    {
        ...
        $this->assertEquals(
            "%kernel.root_dir%/../vendor/maxmind/geoip/src/Maxmind/Bundle/Test/DependencyInjection/GeoIPCity.dat",
            $datFilePath,
...
EOF;
    ...
    private function assertParameter($value, $key)
    {
        $this->assertEquals(
            $value,
            $this->configuration->getParameter($key), sprintf('%s parameter is correct', $key)
        );
    }
